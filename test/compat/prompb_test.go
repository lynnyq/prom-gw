...
func TestCompat_PrometheusOfficial(t *testing.T) {
	...
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{
		Tenant: "app", SourceDC: "dc1", IngestTs: 1700000000000000000,
	})
...
func TestCompat_CortexRemoteWrite(t *testing.T) {
	...
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "cortex", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
	require.Len(t, res.Samples, 1)
	s := res.Samples[0]
...
func TestCompat_VictoriaMetrics_MissingName(t *testing.T) {
	...
	decoded, err := decoder.Decode(body)
	require.NoError(t, err)
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "vm", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
...
func TestCompat_Thanos_NativeHistogramIgnored(t *testing.T) {
	...
	decoded, err := decoder.Decode(body)
	require.NoError(t, err)
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "thanos", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
	require.Len(t, res.Samples, 1)
...
func TestCompat_OpenMetrics_UTF8LabelValue(t *testing.T) {
	...
	decoded, err := decoder.Decode(body)
	require.NoError(t, err)
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "mm", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
	require.Len(t, res.Samples, 1)
...
func TestCompat_LargeLabelValue(t *testing.T) {
	...
	decoded, err := decoder.Decode(body)
	require.NoError(t, err)
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "node", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
	require.Len(t, res.Samples, 1)
	var got string
...
func TestCompat_EmptyWriteRequest(t *testing.T) {
	...
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "noop", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
...
func TestCompat_LargeBatch(t *testing.T) {
	...
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "load", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
...
func TestCompat_MultiSamplesPerSeries(t *testing.T) {
	...
	decoded, err := decoder.Decode(body)
	require.NoError(t, err)
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "m", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
	require.Len(t, res.Samples, 1)
...
func TestCompat_TimestampEdgeCases(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			decoded, err := decoder.Decode(body)
			require.NoError(t, err)
			ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "edge", IngestTs: 1})
			res, err := parser.Parse(ctx, decoded)
			require.NoError(t, err)
			require.Len(t, res.Samples, 1)
...
func TestCompat_DuplicateLabelNames(t *testing.T) {
	...
	decoded, err := decoder.Decode(body)
	require.NoError(t, err)
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "dup", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
	require.Len(t, res.Samples, 1)
...
func TestCompat_LabelsOnlySeries(t *testing.T) {
	...
	decoded, err := decoder.Decode(body)
	require.NoError(t, err)
	ctx := parser.ContextWithMeta(context.Background(), parser.Meta{Tenant: "l", IngestTs: 1})
	res, err := parser.Parse(ctx, decoded)
	require.NoError(t, err)
	assert.Empty(t, res.Samples)
}
