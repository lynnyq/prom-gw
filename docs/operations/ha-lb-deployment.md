...
>
> 配套文档:[生产部署指南](production-guide.md)(含 LVS 方案)、[部署速查](deploy.md)、[压力测试指南](stress-test-guide.md)、[SLO 指标](slo.md)、[故障剧本](runbook.md)

## 目录

1. [高可用架构设计](#1-高可用架构设计)
2. [部署拓扑](#2-部署拓扑)
3. [Nginx 负载均衡配置](#3-nginx-负载均衡配置)
4. [Keepalived 高可用配置](#4-keepalived-高可用配置)
5. [HAProxy 替代方案](#5-haproxy-替代方案)
6. [健康检查机制](#6-健康检查机制)
7. [SSL/TLS 配置](#7-ssltls-配置)
8. [安全加固](#8-安全加固)
9. [多机房容灾](#9-多机房容灾)
10. [故障切换测试](#10-故障切换测试)
11. [监控与告警](#11-监控与告警)
...
- [生产部署指南](production-guide.md) — 含 LVS 负载均衡方案、Kafka 部署、Prometheus 对接
- [部署速查](deploy.md) — 快速部署参考
- [压力测试指南](stress-test-guide.md) — 压测方法与性能基线
- [SLO 指标](slo.md) — 可用性目标与告警分级
- [故障剧本](runbook.md) — 常见故障处理
- [配置参数说明](configuration-reference.md) — prom-gw 全部参数
- [安全规范](../user/auth.md) — 鉴权与安全
