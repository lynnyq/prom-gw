...
sudo chown -R bdops:bdops /appdata/kafka-ui /applog/kafka-ui
```
...
sudo chown -R bdops:bdops /appdata/kafka-ui
...
sudo -u bdops jcmd $PID GC.heap_info

# 查看 JVM 线程栈(排查卡死)
sudo -u bdops jstack $PID | head -100
