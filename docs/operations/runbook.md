...
> 配套文档:`slo.md`(指标定义)、`troubleshooting.md`(具体问题排查)、`deploy.md`(部署/回滚)。
> 本文档聚焦"故障来了怎么办"——按严重程度分级,提供确认 → 隔离 → 恢复 → 复盘 的标准剧本。
...
按 `type` 字段定根因 → 参考 `troubleshooting.md §1`。

### 2.2 p99 延迟高

按 `troubleshooting.md §8` 走:
...
**短期止血**(降级背压):
- 调大 `wal.max_bytes`(临时改配置,SIGHUP 生效)
- 加挂一块盘,迁移 WAL 目录(需停机)
- 扩容 Kafka,加速排空
